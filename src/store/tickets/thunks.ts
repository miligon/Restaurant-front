...
export const refreshTickets = (restaurantSlug: string) => {
    return async (dispatch: Dispatch) => {
        dispatch(setLoading());
        const ticketsList: AxiosResponse<any, any> | string = await getTicketList(restaurantSlug)
...
export const doTicketCreate = (ticket: ticketToServer, restaurant: string) => {
    return async (dispatch: Dispatch) => {
        dispatch(setLoading());
        const response: AxiosResponse<any, any> | string = await createTicketDetail(ticket, restaurant);
...
export const doTicketUpdate = (id: number, ticket: ticketToServer, restaurant: string) => {
    return async (dispatch: Dispatch) => {
        dispatch(setLoading());
        const response: AxiosResponse<any, any> | string = await updateTicketDetail(id, ticket, restaurant);
        if (typeof response !== 'string') {
            if (response.status === 200) {
...
export const doTicketDelete = (id: number) => {
    return async (dispatch: Dispatch) => {
        dispatch(setLoading());
        const response: AxiosResponse<any, any> | string = await deleteTicketDetail(id);
        if (typeof response !== 'string') {
            if (response.status === 200) {
